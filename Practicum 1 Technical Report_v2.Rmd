...
We calculate and input values for beginning weight by subracting pounds gained from body weight. We also calculate change in BMI using a transformation of beginning weight and height.
...
To analyze potential effects of shift and total metabolic minutes simulataneously, we look at a scatterplot with total metabolic minutes as the independent variable and pounds gained as the response variable, with observations colored by shift category.
...
lm = lm(lbs_gained ~ Total_Met_Min, data = dat2)

summary(lm)
```

```{r First Scatterplot, message = FALSE}
ggplot(data = dat2, aes(x = Total_Met_Min, y = lbs_gained)) + geom_point() + geom_smooth()
```
...
```{r Linear Model without outliers, include = FALSE}
lm = lm(lbs_gained ~ Total_Met_Min, data = dat2)
...
```{r Second Scatterplot, message = FALSE}
ggplot(data = dat2, aes(x = Total_Met_Min, y = lbs_gained)) + geom_point() + geom_smooth()
```
