...
mod1 <- glm(weightgain.b ~ shift + gender + Age + height + Total_Met_Min, family = binomial,na.omit(dat1))
...
mod0 <- glm(weightgain.b ~ gender + Age + height + Total_Met_Min, family = binomial,na.omit(dat1))

summary(mod1)
summary(mod0)

anova(mod0, mod1, test = "Chi")
##do not reject the null hypothesis that the reduced model is better(the full is more desirable), therefore we shouldn't include shift in the model
```
